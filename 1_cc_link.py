...
fout = open('output/cc_dd_link.csv','w')
# event pair linkage criteria
...
def calc_cc_dt(event_list, sta_dict, fout):
    ...
    nbr_loader = DataLoader(nbr_dataset, num_workers=0, batch_size=None)
    t = time.time()
    pair_list = []
    ...
    # 2. calc CC & DD
    print('2. calculate waveform CC and diff S-P')
    cd_dataset = CC_DD(event_list, pair_list, sta_dict)
    cd_loader = DataLoader(cd_dataset, num_workers=num_workers, batch_size=None)
    link_num = 0
    t = time.time()
    for i, [[data_evid, temp_evid], cd_dict] in enumerate(cd_loader):
        if i%10000==0: print('done/total {}/{} | {} pairs linked | {:.1f}s'.format(i, num_pairs, link_num, time.time()-t))
        if len(cd_dict)<num_sta_thres: continue
        write_cc_dt(data_evid, temp_evid, cd_dict, fout)
        link_num += 1

def write_cc_dt(data_evid, temp_evid, cd_dict, fout):
    fout.write('{},{}\n'.format(data_evid, temp_evid))
    for net_sta, [dt_sp, dt_p, dt_s, cc_det, cc_p, cc_s] in cd_dict.items():
        fout.write('{},{:.3f},{:.3f},{:.3f},{:.2f},{:.2f},{:.2f}\n'.format(net_sta, dt_sp, dt_p, dt_s, cc_det, cc_p, cc_s))
...
class CC_DD(Dataset):
  """ Dataset for event linking with CC and d(S-P)
  """
  ...
  def __getitem__(self, index):
    ...
    for sta in sta_list:
        ...
        dt_sp = dt_p-dt_s
        if abs(dt_sp)>dt_thres[2]: continue
        cd_dict[sta] = [dt_sp, dt_p, dt_s, cc_det, cc_p, cc_s]
    return [data_evid, temp_evid], cd_dict

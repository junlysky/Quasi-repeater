...
def get_event_list(fpha, event_root):
    ...
    event_pick_list = read_fpha(fpha)
...
def read_data_temp(st_paths, tp, ts, ot):
    ...
    data_det, temp_det, norm_data_det, norm_temp_det = [None]*4
    ...
    and tp-win_temp_det[0]>=st[0].stats.starttime \
    and tp+win_temp_det[1]<=st[0].stats.endtime:
        ...
        data_det = st2np(st.slice(tp-win_data_det[0], tp+win_data_det[1]), npts_data_det)
        temp_det = st2np(st.slice(tp-win_temp_det[0], tp+win_temp_det[1]), npts_temp_det)
        norm_data_det = calc_norm(data_det, npts_temp_det)
        norm_temp_det = np.array([sum(di**2)**0.5 for di in temp_det])
        ttp = tp - ot
    ...
    and ts+max(win_data_s[1],win_temp_s[1])<=st[0].stats.endtime:
        ...
        tts = ts - ot
    data = [data_det, data_p, data_s, norm_data_det, norm_data_p, norm_data_s]
    temp = [temp_det, temp_p, temp_s, norm_temp_det, norm_temp_p, norm_temp_s]
...
def read_fpha(fpha):
    ...
    for line in lines:
        ...
        if len(codes[0])>=10:
            ...
            if ot_min<ot<ot_max and lat_min<lat<lat_max and lon_min<lon<lon_max: to_add = True
...
def read_fpha_dict(fpha):
    ...
    for line in lines:
        ...
        if len(codes[0])>10:
            ...
            ot = codes[0]
            lat, lon, dep, mag = [float(code) for code in codes[1:5]]
            event_line = '%s,%s,%s,%s,%s,%s\n'%(ot,lat,lon,dep,mag,evid)
            event_dict[evid] = [[lat, lon, dep, mag], event_line, []]
        else: event_dict[evid][-1].append(line)
    return event_dict

def read_ftemp_dict(fpha):
    event_dict = {}
    f=open(fpha); lines=f.readlines(); f.close()
    for line in lines:
        codes = line.split(',')
